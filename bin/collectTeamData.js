  ...
  .then((res) => {
    ...
    rows.forEach((row, index) => {
      if (row[0] === "CATEGORY") {
        ...
        const imageProcessing = fetch(row[CATEGORY_FIELD_TO_COL.imageUrl])
          .then((res) => {
            if (!res.ok) {
              if (res.status === 403) {
    ...
    Promise.all(categoryImagePromises).then(() => {
      console.log(`${categoryImagePromises.length} category images saved`);
    });
      ...
      for (
        ...
        if (row[0] !== "" && row[LEADERSHIP_FIELD_TO_COL["display"]] === "Y") {
          ...
          if (props.imageUrl) {
            ...
            hasDefaultImages = true;
            // we don't need to include public/ since it is resolved by default
            personData[category.name].people[myRow].imageUrl = path.posix.join(
              LEADERSHIP,
              DEFAULT_FILENAME
            );
        ...
        console.log(
          `${
            Object.keys(personData).reduce(
              (sum, category) => sum + personData[category].people.length,
              0
